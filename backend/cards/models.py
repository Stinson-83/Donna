"""Donna card models v1 — vendored backend twin of
`donna-design-spec/schema/card.schema.json` (and its `schema/models.py`).

Validate every model-emitted payload with `DonnaCard.model_validate(payload)`.
On ValidationError: fall back to sending body text as a plain message — never a
broken card. Keep this file checked against the JSON Schema in CI so the frontend
TS types and these models can never drift.

DO NOT edit the block vocabulary here without changing the design-spec schema
first — that file is the contract.
"""
